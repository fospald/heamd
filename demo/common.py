...
class Experiment(object):
	...
	def add_result(self, key, record=None):
		...
		key_map = {
			"solve_time":  (hm.get_solve_time, True),
			"mean_stress": (hm.get_mean_stress, True),
			"mean_strain": (hm.get_mean_strain, True),
			"A2": (hm.get_A2, True),
			"A4": (hm.get_A4, True),
			"iterations":  (lambda: len(hm.get_residuals()), True),
			"residuals":   (hm.get_residuals, False),
		}
	...
	def run_experiment(self, assignments, results, version, dry, cache_only):
		...
		hm.reset()
		hm.set_xml_precision(5)
		hm.load_xml(self.project_xml)
		hm.set("results", ",".join([r[0] for r in results]))
		...
		hm.erase("num_threads")
		hm.erase("solver.fft_planner_flag")
	...
	def add_fibers(self):
		with open(self.fiber_file, 'rb') as csv_file:
			...
			for row in csv_reader:
				...
				R = row[6]
				L0 = L + (4.0/3.0)*R
				hm.set("actions.%s.place_fiber[%d]." % (self.fiber_group, nfibers),
...
def reset_hm(filename='project.xml'):
	hm.reset()
	hm.load_xml(filename)
...
def set_params_by_contrast(gamma, k_1_ge_1 = False):
	...
	hm.set("solver.materials.mat1..mu", mu_1);
	hm.set("solver.materials.mat1..lambda", lambda_1);
	hm.set("solver.materials.mat2..mu", mu_2);
	hm.set("solver.materials.mat2..lambda", lambda_2);
	hm.set("solver.materials.matrix..mu", mu_3);
	hm.set("solver.materials.matrix..lambda", lambda_3);
	...
	if (False and k_1_ge_1):
		...
		hm.set("solver.materials.ref..mu", ref)
		hm.set("solver.materials.ref..lambda", ref)
...
def new_HM():
	hm = heamd.HM()
	hm.set_py_enabled(False)
	return hm

print_max_problem_sizes()

# create heamd instance
hm = new_HM()
...
def inspect():
	hm_gui = __import__("heamd_gui")
	app = hm_gui.App(sys.argv)
	app.window.runProject(hm)
	app.exec_()
